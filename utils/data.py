...
class iImageNet1000(iData):
    ...
    def download_data(self):
        train_dir = "../my_data/imagenet1000/train/"
        test_dir = "../my_data/imagenet1000/test/"
...
class iImageNet100(iData):
    ...
    def _align_class_labels(self, train_class_to_idx, test_class_to_idx):
        """
        Remap test class indices to match train class indices.
        Assumes train and test have the SAME classes.
        """
        mapping = {}
        for class_name, test_idx in test_class_to_idx.items():
            train_idx = train_class_to_idx[class_name]  # Will fail if class missing
            mapping[test_idx] = train_idx

        self.test_targets = np.array([mapping[t] for t in self.test_targets])
...
#     def download_data(self):
#         # check if directories exist. If not, update to match your setup:
