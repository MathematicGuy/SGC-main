...
def accuracy(y_pred, y_true, nb_old, increment=10, class_groups=None): # Thêm class_groups
    ...
    if class_groups:
        logging.info("Calculating accuracy per defined class group.")
        for group_id, classes_in_group in class_groups.items():
            idxes = np.where(np.isin(y_true, classes_in_group))[0]
        ...
        logging.info(f"Calculating accuracy per increment of {increment}.")
